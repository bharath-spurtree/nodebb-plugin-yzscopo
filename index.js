...
API.init = function (data, callback) {
	...
	data.router.use('/api/v5', routes.v5);
...
API.authenticate = async (data) => {
	await require('./routes/v5/middleware').requireUser(data.req, data.res, data.next);
};

API.associateUser = require('./routes/v5/middleware').associateUser;

API.reloadSettings = function (hash) {
	if (!hash || hash === 'settings:writeapi') {
		meta.settings.get('writeapi', function (err, settings) {
			if (err) {
				winston.warn('[plugins/yzscopo] Unable to reload settings');
			}
